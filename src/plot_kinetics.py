...
def savefig(name, suffix=suffix):
...
inter_probability_cmap = sns.cubehelix_palette(start=1.5, rot=-0.75, dark=0.15, light=1.0, hue=1, gamma=, as_cmap=True)
...
def _plot_states(seq, paired_probabilities, states, *, seqfs=8, revseq=False,
                 vmin=0, vmax=1, cmap = inter_probability_cmap, **kwargs):
...
def _dotplot(seqA, seqB, state_probabilities, pair_probabilities, *,
             ...
             cmap=inter_probability_cmap, vmin=0, vmax=None, **kwargs):
...
class RikinRun:
    ...
    * plots the data in various ways
    ...
    def plot_states(self, figsize, seqfs, hspace=1, wspace=0.2, cbar_ratio=0.025, **kwargs):
        ...
        for dim in [0,1]:
            ...
            _plot_states(self.seqs[dim], interpaired, states=self.shown_states, revseq=(dim==1),
                         vmin = vmin, vmax = vmax,
                         ax=ax[f'i{dim}'], cbar_ax=ax['ibar'], **kwargs)
            ...
            _plot_states(self.seqs[dim], intrapaired, states=self.shown_states,
                         ...
                         ax=ax[f's{dim}'], cbar_ax=ax['sbar'], **kwargs)
    ...
    def intramolecular_dotplot(self, dim, **kwargs):
        ...
        return _dotplot(self.seqs[dim], self.seqs[dim], self.state_probabilities, pair_probabilities,
                        cmap = inter_probability_cmap, **kwargs)
    ...
    def dotplot(self, *, figsize, barsize=0.5, seqfs=8, vmax=1, space=0.5, hspace=None, wspace=None, **kwargs):
        ...
        '''
        fig,ax = plt.subplot_mosaic(layout, figsize=figsize, 
                                    width_ratios=[lenA,lenB,bar_ratio], height_ratios=[lenA,lenB],
                                    gridspec_kw = dict(hspace=hspace,wspace=wspace),
                                    **kwargs)

        self.intramolecular_dotplot(0, seqfs=seqfs,cbar=True, cbar_ax=ax['b'], vmax=vmax, ax=ax['A'])
        self.intramolecular_dotplot(1, seqfs=seqfs, cbar=False, vmax=vmax,ax=ax['B'])
        self.interaction_dotplot(seqfs=seqfs, vmax=vmax, cbar=False, ax=ax['I'])
...
micA_micA.plot_states(figsize=(12,4), seqfs=6, wspace=0.15, hspace=0.6 )
plt.show()
...
micA_micA.dotplot(figsize = (12,12), seqfs = 6, barsize=0.25, space=0.25, hspace=0.15)
plt.show()
# -
...
for a,b in [(1,2),(2,3)]:
    ...
    khp.plot_states(figsize=(8,3),seqfs=8)
    plt.show()
    
    khp.plot_paired_probability_kinetics(figsize=(8,8), seqfs=8,wspace=0.5,hspace=0.2)
    plt.show()
    
    khp.dotplot(figsize=(6.5,6),seqfs=8,space=0.35)
    plt.show()
