#ifndef ORDERED_SPARSE_MATRIX_HH
#define ORDERED_SPARSE_MATRIX_HH
...
//namespace LocARNA {
    ...
    /**
     * \brief Represents a sparse 2D matrix using (ordered) map
     *
     * Sparse matrix of entries val_t implements the matrix by an
     * ordered map. The class is designed to be largely exchangable
     * with the non-sparse Matrix class. (A proxy class is used to
     * provide the same syntax for the interface.)
     *
     * @see Matrix
     *
     * @todo make locarna sparse map more general to support ordered
     * and unordered maps
     */
    ...
    public:
  	...
  	typedef _value_t value_t; //!< type of matrix entries
	...
	struct key_compare {
	    bool
	    operator () (const key_t &x,const key_t &y) const {
		return x.first<y.first || 
			       (x.first==y.first && x.second<y.second);
	    }
	};
		
	typedef std::map<key_t, value_t, key_compare> map_t; //!<map type 
	...
	class element {
	private:
	    OrderedSparseMatrix<value_t> *m_;
	    key_t k_;
	public:
	    /** 
	     ...
	     *
	     */
	    element(OrderedSparseMatrix<value_t> *m,key_t k): m_(m),k_(k) {}
	 ...
	 */
	OrderedSparseMatrix() : the_map_(),def_() {}
	 ...
	 */
	OrderedSparseMatrix(const value_t &def) : the_map_(),def_(def) {}
    ...
    inline
    std::ostream &
    operator <<(std::ostream &out, const OrderedSparseMatrix<value_t> &m) {
	for (typename OrderedSparseMatrix<value_t>::const_iterator it=m.begin();
	     m.end()!=it; 
	     ++it) {
...
#endif // ORDERED_SPARSE_MATRIX_HH
